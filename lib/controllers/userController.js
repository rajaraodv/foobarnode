    ...
    function createUser(req, res) {
        var b = req.body;
        var access_token = crypto.createHash('md5').update(b.username + b.account_id + 'secretWordCF').digest('hex');
        if(!b.username || !b.account_id) {
            res.json(401, {
                 'Error': 'Missing fields - Both username and account_id are required.'
             });
             return;
        }
        var post_data = {
        ...
        User.findByUserNameAndAccessToken(post_data.username, post_data.access_token, function(err, user) {
            if(err) {
                ...
                // });
                //
                // Be flexible and simply send existing user instead of an error.
                res.json(user);
            } else {
