    ...
    module.exports = function(app, m, service) {
        ...
        User = useObj.User; //store
        UserSchema = useObj.UserSchema;//store
        //create
        app.post('/users', service.validateAndLoadUser, createUser);
        //read
        app.get('/users/:id', service.validateAndLoadUser, readUser);
        //read
        app.get('/users', service.validateAndLoadUser, readUser);
        //update
    ...
    function updateUser(req, res) {
        ...
        var updateObj = User.getUpdateObject(UserSchema.paths, user, b);
        if(new_access_token) {
    ...
    function readUser(req, res) {
        var id = req.params.id;
        var user = req.user;
        if(!id || id == user._id.toString()) { //current user
            res.json(req.user);
            return;
        }
        User.findOne({
